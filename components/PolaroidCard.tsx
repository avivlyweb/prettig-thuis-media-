...
const ErrorDisplay = ({ title, message }: { title: string; message: string; }) => (
    <div className="flex flex-col items-center justify-center h-full text-center p-4">
        ...
        <p className="text-base text-red-600 mt-1">{message}</p>
    </div>
);
...
const Placeholder = ({ isActivityStrip }: { isActivityStrip?: boolean }) => (
     <div className={`flex flex-col items-center justify-center h-full text-neutral-500 ${!isActivityStrip && 'group-hover:text-neutral-600'} transition-colors duration-300`}>
        ...
        <span className="font-permanent-marker text-xl">Upload een Foto</span>
    </div>
);

const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, isActivityStrip }) => {
    ...
    return (
        <div className={cardClasses}>
            <div className="w-full bg-gray-200 shadow-inner flex-grow relative overflow-hidden rounded-sm">
                ...
                {status === 'error' && <ErrorDisplay title={error?.title || 'Sorry'} message={error?.message || 'er is iets misgegaan.'} />}
